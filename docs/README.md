...
2.  **Import to Vercel**:
    ...
    *   Import your `insurance-claim-ai` repository.
3.  **Configure Project**:
    *   **Framework Preset**: Select **Next.js**.
    *   **Root Directory**: `frontend` (Important: The UI lives here).
    *   **Environment Variables**: Add your `OPENAI_API_KEY`.
4.  **Deploy Backend**:
    *   For a full stack deployment, we recommend deploying the `backend` separately (e.g., on [Render](https://render.com) or [Railway](https://railway.app)) as a Python service.
    *   Update `frontend/.env` with `NEXT_PUBLIC_API_URL` pointing to your deployed backend.
